...
func parseFields(event *parser.Event) ParsedEvent {
	...
	for _, v := range event.Fields {
		...
		case "bifrost_primitives.currency.CurrencyId.input_asset":
			parsedEvent.InputAsset = fmt.Sprint(v.Value)
			if decodedFields, ok := v.Value.(registry.DecodedFields); ok {
				if len(decodedFields) > 0 {
					reflect.TypeOf(decodedFields[0].Value)
					parsedEvent.InputAsset = fmt.Sprint(&decodedFields[0].Value)
				}
			}
		case "bifrost_primitives.currency.CurrencyId.output_asset":
			parsedEvent.OutputAsset = fmt.Sprint(&v.Value)
		case "input_amount":
